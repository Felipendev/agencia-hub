...
const number = parseCurrencyInput;
...
export default function VendasPage() {
  ...
  function openEditReceivable(row: ReceivableRow) {
    ...
    setEditAmount(centsToDisplay(row.amount));
  ...
  return <div className="space-y-6">
    ...
    {isRevenueRegistration && <form onSubmit={submit} className="space-y-4 rounded-xl border border-[var(--hub-border)] bg-white p-5">
      ...
      <div className="space-y-2 rounded-lg bg-[var(--hub-bg-subtle)] p-3">
        ...
        {installments.map((installment, index) => <div key={index} className="grid gap-2 sm:grid-cols-3"><CurrencyInput className={attempted && number(installment.amount) <= 0 ? "border-red-500" : ""} value={installment.amount} onValueChange={(formatted) => setInstallments((current) => current.map((item, position) => position === index ? { ...item, amount: formatted } : item))} placeholder={`Valor da parcela ${index + 1}`} /><Input className={attempted && !installment.dueDate ? "border-red-500" : ""} type="date" value={installment.dueDate} onChange={(event) => setInstallments((current) => current.map((item, position) => position === index ? { ...item, dueDate: event.target.value } : item))} aria-label={`Data prevista da parcela ${index + 1}`} />{installments.length > 1 && <Button type="button" size="sm" variant="ghost" className="text-red-600" onClick={() => setInstallments((current) => current.filter((_, position) => position !== index))}>Remover</Button>}</div>)}
      </div>
      ...
      {advanced && <div className="space-y-5 rounded-lg border border-[var(--hub-border)] bg-[var(--hub-bg-subtle)] p-4">
        <div>
          ...
          <div className="space-y-3">
            {lines.map((line, index) => <div key={index} className="grid gap-2 rounded-md bg-white p-3 sm:grid-cols-6"><Input value={line.description} onChange={(event) => updateLine(index, { description: event.target.value })} placeholder="Ex.: passagem aérea" /><CurrencyInput value={line.saleAmount} onValueChange={(formatted) => updateLine(index, { saleAmount: formatted })} placeholder="Valor deste item" /><Select value={line.supplierId} onChange={(event) => updateLine(index, { supplierId: event.target.value })}><option value="">Sem fornecedor</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</Select><CurrencyInput value={line.supplierCost} onValueChange={(formatted) => updateLine(index, { supplierCost: formatted })} placeholder="Custo do fornecedor" /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={line.direct} onChange={(event) => updateLine(index, { direct: event.target.checked })} /> Cliente paga direto</label><Button type="button" size="sm" variant="ghost" className="text-red-600" onClick={() => setLines((current) => current.filter((_, position) => position !== index))}>Remover</Button></div>)}
          </div>
