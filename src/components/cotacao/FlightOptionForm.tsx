...
export function FlightOptionForm({ option: o, tables, principal, onPrincipal, onChange, onRemove }: {
  ...
  return <section className="space-y-4 rounded-xl border border-[var(--hub-border)] bg-white p-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <label className="flex items-center gap-2 text-sm"><input type="radio" name="opcao-principal" checked={principal} disabled={!o.incluir} onChange={onPrincipal} />Usar no total da cotação</label>
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      <label className="text-sm">Nome da opção (obrigatório)<Input value={o.nome} maxLength={150} onChange={(e) => set({ nome: e.target.value })} /></label>
      ...
      <label className="text-sm">Passageiros com o mesmo preço<Input type="number" min={1} max={20} value={o.qtdPessoas} onChange={(e) => set({ qtdPessoas: Number(e.target.value) })} /></label>
    </div>
    ...
    {o.segmentos.map((s, index) => <fieldset key={index} className="rounded-lg border border-[var(--hub-border)] p-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="text-sm">Duração (minutos)<Input type="number" min={1} max={10080} placeholder="355 = 5h55" value={s.durationMinutes ?? ""} onChange={(e) => segment(index, { durationMinutes: e.target.value === "" ? null : Number(e.target.value) })} /></label>
    ...
    <label className="flex items-start gap-2 text-sm"><input className="mt-1" type="checkbox" checked={o.calculo.revisado} onChange={(e) => onChange({ ...o, calculo: { ...o.calculo, revisado: e.target.checked } })} />Conferi os dados preenchidos e o preço. Campos vazios aparecerão como não informados para o cliente.</label>
  </section>;
}
